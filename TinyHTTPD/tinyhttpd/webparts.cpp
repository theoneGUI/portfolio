...
// WebDoc class implementation

WebDoc::WebDoc()
	: documentContent{ "" }, loadsFromFile{ false }, loadsFileIntoMemory{ false }, mimeType{ "text/html" }
{}

WebDoc::WebDoc(std::string content)
	: documentContent{ content }, loadsFromFile{ false }, loadsFileIntoMemory{ false }, mimeType{ "text/html" }
{
}

WebDoc::WebDoc(webfile path, bool loadIntoMemory, const string& mime) {
	filePath = path;
	loadsFromFile = true;
	loadsFileIntoMemory = loadIntoMemory;
	mimeType = mime;
	if (loadIntoMemory) {
		ifstream fileLen(filePath, ios::ate | ios::binary);
		size_t length = fileLen.tellg();
		fileLen.close();
...
bool WebDoc::operator==(const WebDoc& comp) const {
	return comp.documentContent == this->documentContent && comp.mimeType == this->mimeType;
}
...
std::string Response::buildResponse() {
	...
	resp += "Content-Type: " + document.mimeType + "\r\n";
...
std::string Response::buildResponse(short statusCode, const std::unordered_map<std::string, std::string>& headers, WebDoc doc) {
	...
	resp += statusCode;
	resp += "\r\n";
	resp += "Content-Type: "+doc.mimeType;
	resp += "\r\n";
	for (const auto& i : headers) {
		resp += i.first + ": " + i.second + "\r\n";
	}
...
// WebDir class implementation

WebDir::WebDir()
	: documentContent{ "" }
{}

WebDir::WebDir(webdir path) {
	dirPath = path;
}

std::string WebDir::getContent(string relative) {
	string filePath = dirPath + "/" + relative;
	...
	file.close();
	documentContent = fileContents;
	return fileContents;
}

const char* WebDir::c_str() const {
	return this->documentContent.c_str();
}

size_t WebDir::length() const {
	return this->documentContent.length();
}

bool WebDir::operator==(const WebDir& comp) const {
	return comp.dirPath == this->dirPath;
}
